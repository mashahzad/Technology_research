...
void main(void)
{
  ...
  double queue_size = QUEUE_MAX_USER/packet_size_Kbytes; // queue size limit for given transmission and packet size (how much data fills queue)
  ...
  while (time < end_time)
  {
    if (t1 < t2)                 // *** Event #1 (arrival)
    {
	  ...
	  if(n > 1)
	  {
	      if(n <= (queue_size+1)) //total packets data in system = cusomers in queue + 1 in service
            {
